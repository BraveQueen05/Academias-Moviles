package pe.flavia.optionaltwo.entities

import android.os.Parcelable
import kotlinx.parcelize.IgnoredOnParcel
import kotlinx.parcelize.Parcelize
import pe.flavia.optionaltwo.intoCurrency
...
@Parcelize
class Vegetable(val id: Int = -1, val name: String = "", val price: Double = 0.0, val description: String = "", val img: String = "") :
    Parcelable {
    @IgnoredOnParcel
...
enum class EVegetables(val id: Int, val vName: String, val price: Double, val description: String, val img: String){
    ...
    companion object{
        fun getVegetablesList(): MutableList<Vegetable> {
            return values().map { Vegetable(it.id, it.vName, it.price, it.description, it.img) }.toMutableList()
        }
    }
}